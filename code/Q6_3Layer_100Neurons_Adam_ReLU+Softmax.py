...
fig1 = plt.plot(np.arange(0,len(loss_step),1),loss_step,color='red')
plt.title('Loss over steps(' + str(len(loss_step)) + ')')
plt.xlabel("steps")
plt.ylabel("error")
plt.show()
...
print('Misclassification of the network on the 10000 test images: %d %%' % (100 - 100 * correct / total))
# --------------------------------------------------------------------------------------------
_, predicted = torch.max(outputs.data, 1)
